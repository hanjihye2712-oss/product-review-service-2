...
class ReviewSimilarityResult(models.Model):
    ...
    source_review = models.ForeignKey(
        "reviews.Review",
        on_delete=models.CASCADE,
        related_name="similarity_results_as_source",
    )
    compared_review = models.ForeignKey(
        "reviews.Review",
        on_delete=models.CASCADE,
        related_name="similarity_results_as_compared",
    )
    compared_username_snapshot = models.CharField(max_length=150, blank=True)
    similarity_score = models.FloatField()
    similarity_label = models.CharField(max_length=50, blank=True)
    ...
    def __str__(self):
        return f"{self.source_review_id} vs {self.compared_review_id} ({self.similarity_score:.2f})"
